  ...
  <div>
    <div v-if="isAuthenticated">
      ...
      <div v-masonry transition-duration="1s" item-selector=".item" class="row">
        <div v-masonry-tile class="item col-4 my-2" v-for="user in users" v-bind:key="user.id">
          <a class="card" v-bind:href="'https://twitter.com/' + user.screen_name"
            ...
            <div class="card-body">
              <div v-if="user.location">
                <small class="text-muted">{{ user.location }}</small><br>
              </div>
              <h5 class="card-title">{{ user.name }}
                <small class="text-muted">@{{ user.screen_name }}</small>
              </h5>
              <hr>
              <div v-if="user.description">{{ user.description }}<br></div>
              <small class="text-muted" v-for="keyword in user.keywords"
                v-bind:key="keyword.id">{{keyword}} </small>
            </div>
